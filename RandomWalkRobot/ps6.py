...
class Robot(BaseRobot):
    ...
    def updatePositionAndClean(self):
        ...
        new_p = self.getRobotPosition().getNewPosition(self.getRobotDirection(), self.speed)
...
##    print(all_trials_progress_list)
    return all_trials_progress_list

a=runSimulation(2, 1.0, 5, 5, 1.0, 3, Robot, True)
...
def showPlot1():
    ...
    pylab.figure()
    pylab.title('Time to clean 75% of a square room with 2 robots, for various room areas')
    pylab.xlabel('semilogx()')
    pylab.ylabel('semilogy()')
    pylab.plot(X,Y,'ro')
    pylab.semilogx()
    pylab.semilogy()
    pylab.figure()
...
def showPlot4():
    ...
    """
    # ascending, overlaps
    coverages = pylab.np.linspace(0.1, 1.0, 11)
...
##runSimulation(2, 1.0, 5, 5, 1.0, 10, RandomWalkRobot, True)
##
# === Problem 6
...
def showPlot6():
    ...
    Y = []
    for width, height in rooms:
        lol = runSimulation(2, 1.0, width, height, 0.75, 25, RandomWalkRobot, False)
        Y.append(get_average_time(lol))
    pylab.plot(X, Y)
    pylab.xlabel('Room shape')
    pylab.ylabel('Timesteps')
...
def showPlot7():
    ...
    for nb, color in [(1, 'blue'), (2, 'red'), (3, 'green'), (4, 'yellow'), (5, 'black')]:
        Y = []
        for coverage in coverages:
            lol = runSimulation(nb, 1.0, 25, 25, coverage, 25, RandomWalkRobot, False)
            Y.append(get_average_time(lol))
        pylab.plot(coverages, Y, color=color, label='%d robots' % nb)
    pylab.xlabel('Percentage cleaned')
    pylab.ylabel('Timesteps')
